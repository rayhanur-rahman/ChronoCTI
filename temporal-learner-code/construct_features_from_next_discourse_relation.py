...
PATH_TO_NEXT_DISCOURSE_RELATION_FEATURE = 'unseen_reports_features/unseen_reports_next_sentence_discourse_features_95.json' # Features_v2/next_sentence_discourse_features_{prediction_threshold}_v2.json


dataset_discourse_relation = json.load(open(f'{PATH_TO_NEXT_SENTENCE_PREDICTION}'))
...
for prediction_threshold in range(95, 100, 5):
    ...
    for idx in tqdm.tqdm(range(len(dataset_discourse_relation))):
        ...
        for idx in range(len(ttp_pairs)):
            ...
            if ttp_probs[f"{te1['id']}"] >= prediction_threshold and ttp_probs[f"{te2['id']}"] >= prediction_threshold:
                ...
                # if len(sentences_having_te1) > 0 and len(sentences_having_te2) > 0:
                #     print('...')
    ...
    json.dump( all_feature_sets, open( f'{PATH_TO_NEXT_DISCOURSE_RELATION_FEATURE}', 'w' ) )